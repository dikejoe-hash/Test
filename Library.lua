...
local function InitJWareUI()
	...
	local function CreateWindow(opts)
		...
		local WIN_W = opts.Size.X
		local WIN_H = opts.Size.Y
		...
		local function trackOutlineStroke(stroke)
			table.insert(themedElements.outlineStrokes, stroke)
			return stroke
		end
		local function trackGradient(grad)
			table.insert(themedElements.gradients, grad)
			return grad
		end
		local function trackFill(frame)
			table.insert(themedElements.fills, frame)
			return frame
		end
		local function trackScroll(sf)
			table.insert(themedElements.scrolls, sf)
			return sf
		end
		local function trackMainFrame(frame)
			table.insert(themedElements.mainFrames, frame)
			return frame
		end
		local function trackBg2Frame(frame)
			table.insert(themedElements.bg2Frames, frame)
			return frame
		end
		local function trackTabButton(lbl)
			table.insert(themedElements.tabButtons, lbl)
			return lbl
		end
			...
			for _, g in ipairs(themedElements.gradients) do
				if g and g.Parent then
					g.Color = ColorSequence.new{
						ColorSequenceKeypoint.new(0, newTheme.MainColor),
						ColorSequenceKeypoint.new(1, newTheme.BackgroundColor2),
					}
				end
			end
		...
		MainFrame.Position         = UDim2.fromOffset(
			math.floor(VIEWPORT.X / 2 - WIN_W / 2),
			math.floor(VIEWPORT.Y / 2 - WIN_H / 2)
		)
		MainFrame.Parent = mainGui
		trackMainFrame(MainFrame)
		addStrokeTracked(MainFrame, Theme.MainColor, 2)
		...
		trackMainFrame(TitleBar)

		local TitleLabel = Instance.new("TextLabel")
		TitleLabel.Name               = "Title"
		TitleLabel.BackgroundTransparency = 1
		TitleLabel.Size               = UDim2.new(0, WIN_W - 20, 1, 0)
		...
		ContentFrame.Size             = UDim2.new(0, WIN_W - 10, 0, WIN_H - 35)
		ContentFrame.Position         = UDim2.new(0, 5, 0, 30)
		ContentFrame.Parent           = MainFrame
		trackBg2Frame(ContentFrame)
		...
		trackMainFrame(KeybindFrame)
		addStrokeTracked(KeybindFrame, Theme.MainColor, 2)
		...
		trackMainFrame(WatermarkFrame)
		addStrokeTracked(WatermarkFrame, Theme.MainColor, 2)
		...
		trackBg2Frame(WatermarkContent)
		...
		function Window:AddTab(tabOpts)
			...
			function Tab:Activate()
				...
				self.Active = true
				tween(TabButton, { TextColor3 = COL_TEXT_HOVER, BackgroundTransparency = 0, BackgroundColor3 = Theme.MainColor })
			...
			function Tab:AddSection(secOpts)
				...
				SectionFrame.Size             = UDim2.new(0, 218, 0, 40)
				SectionFrame.Parent           = Column
				trackBg2Frame(SectionFrame)
				...
				GradGrad.Transparency = NumberSequence.new{
					...
					local function syncCheck(animated)
						if toggled then
							if not table.find(themedElements.activeChecks, Check) then
								table.insert(themedElements.activeChecks, Check)
							end
						else
							local idx = table.find(themedElements.activeChecks, Check)
							if idx then table.remove(themedElements.activeChecks, idx) end
						end
					...
					if cfg.Multi then
						for _, ob in ipairs(optButtons) do
							if table.find(selected, ob.Name) then
								...
								if not table.find(themedElements.activeOptLabels, ob.Label) then
									table.insert(themedElements.activeOptLabels, ob.Label)
								end
							end
						end
					end
				...
				function Section:AddColorPicker(cfg)
					...
					addStrokeTracked(PickerPanel, Theme.MainColor, 1)
					...
					return {
						...
						SetColor = function(c) initFromColor(c) end,
					}
				end
				...
				function Section:AddKeyPicker(cfg)
					...
					KeyLbl.Position               = UDim2.new(1, -5, 0, 0)
